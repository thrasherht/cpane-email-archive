...
VERSION=0.1.0
USAGE="Usage: email-archive.sh [username] [domain] [Older then days]"
...
if [ $SENT == true ] ; then
        ...
        do
                BACKUP_DIR="/backup/emailarchive/$DATE/$i/Sent"
                mkdir -p $BACKUP_DIR
                ...
                find $SENT_DIR/cur/ -type f -mtime +$3 -exec cp -a {} $BACKUP_DIR \; 2>/dev/null
                find $SENT_DIR/new/ -type f -mtime +$3 -exec cp -a {} $BACKUP_DIR \; 2>/dev/null
                    fi
        done
fi
...
if [ $INBOX == true ] ; then
        for i in `ls /home/$1/mail/$2`;
        do
                BACKUP_DIR="/backup/emailarchive/$DATE/$i/Archive"
                mkdir -p $BACKUP_DIR
                echo "Created backup directory at $BACKUP_DIR"
                    if [ $MV == true ] ; then
                ...
                find $ARCHIVE_DIR/cur/ -type f -mtime +$3 -exec cp -a {} $BACKUP_DIR \; 2>/dev/null
                find $ARCHIVE_DIR/new/ -type f -mtime +$3 -exec cp -a {} $BACKUP_DIR \; 2>/dev/null
                    fi
        done
fi
